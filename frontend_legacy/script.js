...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initialize() {
        ...
        resetChoice(newProvinceChoices, t('newProvinceLoading', 'Đang tải tỉnh/thành...'));
        resetChoice(newCommuneChoices, t('newCommunePlaceholder'));

        addEventListeners();
        loadOldDataDropdowns(); //Gọi địa chỉ cũ
        loadNewProvincesDropdown(); //Gọi địa chỉ mới
    ...
    async function loadNewProvincesDropdown() {
        try {
            ...
            if (!response.ok) throw new Error(t('errorFetchNewProvinces', 'Lỗi tải danh sách tỉnh/thành mới.'));
            let data = await response.json();
            if (!Array.isArray(data)) throw new Error('Dữ liệu không phải mảng.');
            const localizedData = data.map(province => ({
                ...province,
                name: localize(province.new_province_name, province.new_province_en_name)
            }));
            updateChoices(newProvinceChoices, t('newProvincePlaceholder', 'Chọn Tỉnh/Thành phố (Mới)'), localizedData, 'new_province_code', 'name');
            enableChoice(newProvinceChoices, newProvinceSelectEl);
        } catch (error) {
            console.error('Lỗi loadNewProvincesDropdown:', error);
            resetChoice(newProvinceChoices, t('newProvinceError', 'Lỗi tải danh sách tỉnh/thành.'));
        }
    }
    ...
    // GHI CHÚ: KẾT THÚC THAY ĐỔI
        ...
        if (newProvinceSelectEl) {
            // Dùng flag để tránh handler 'choice' và 'change' chạy 2 lần liên tiếp
            let _newProvinceHandling = false;

            const handleNewProvinceChange = async (event) => {
                // Choices.js bắn cả 'choice' và 'change' — chỉ xử lý lần đầu
                if (_newProvinceHandling) return;
                _newProvinceHandling = true;
                setTimeout(() => { _newProvinceHandling = false; }, 200);

                // Đọc giá trị theo đúng cách Choices.js cung cấp (như forward lookup)
                const provinceCode = event.detail?.value;
                if (!provinceCode) return;

                resetChoice(newCommuneChoices, t('newCommuneLoading', 'Đang tải phường/xã...'));
                lookupBtn.disabled = true;

                try {
                    const apiUrl = `/api/new-geo-data?province_code=${encodeURIComponent(provinceCode)}`;
                    const response = await fetch(apiUrl);
                    if (!response.ok) throw new Error(t('newCommuneError', 'Lỗi tải danh sách phường/xã mới.'));
                    ...
                    const localizedData = data.map(ward => ({
                        ...ward,
                        name: localize(ward.new_ward_name, ward.new_ward_en_name)
                    }));

                    updateChoices(newCommuneChoices, t('newCommunePlaceholder', 'Chọn Phường/Xã (Mới)'), localizedData, 'new_ward_code', 'name');
                    enableChoice(newCommuneChoices, newCommuneSelectEl);
                } catch (error) {
                    console.error('Lỗi tải danh sách xã mới:', error);
                    resetChoice(newCommuneChoices, t('newCommuneError', 'Lỗi tải.'));
                    showNotification(error.message, 'error');
                }
            };
        ...
        if (newCommuneSelectEl) {
            const handleNewCommuneChange = (event) => {
                const val = event.detail?.value ?? event.detail?.choice?.value ?? newCommuneChoices.getValue(true);
                lookupBtn.disabled = !val;
            };
            newCommuneSelectEl.addEventListener('choice', handleNewCommuneChange);
            newCommuneSelectEl.addEventListener('change', handleNewCommuneChange);
        }
