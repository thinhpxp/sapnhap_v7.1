...
export async function lookupRoute(fastify) {
    fastify.get('/api/lookup', {
        ...
        try {
            if (type === 'forward') {
                const eventsRes = await queryWithCircuitBreaker(`SELECT * FROM merger_events WHERE old_ward_code = $1`, [wardCode]);
                const villagesRes = await queryWithCircuitBreaker(`SELECT * FROM village_changes WHERE old_ward_code = $1`, [wardCode]);
                const result = {
                ...
                const eventsRes = await queryWithCircuitBreaker(`SELECT * FROM merger_events WHERE new_ward_code = $1`, [wardCode]);
                const events = eventsRes.rows;
                const result = await Promise.all(events.map(async (event) => {
                    const villagesRes = await queryWithCircuitBreaker(`SELECT * FROM village_changes WHERE old_ward_code = $1`, [event.old_ward_code]);
                    return {
                        ...event,
                        village_changes: villagesRes.rows,
                    };
