...
async function verifyTurnstileToken(token, remoteIp) {
    ...
    if (!secret)
        return true; // Skip if Turnstile secret is not set in env
    try {
...
export async function feedbackRoute(fastify) {
    ...
    fastify.post('/api/feedback', {
        ...
        try {
            ...
            await queryWithCircuitBreaker(`INSERT INTO feedback (message, context) VALUES ($1, $2)`, [message.trim(), context ? JSON.stringify(context) : null]);
    ...
    fastify.get('/api/feedback', async (request, reply) => {
        ...
        try {
            const selectRes = await queryWithCircuitBreaker(`SELECT id, message, created_at, context FROM feedback WHERE is_sent_to_telegram = FALSE ORDER BY created_at ASC`);
            ...
            messages.forEach((msg) => {
                ...
                if (msg.context && Object.keys(msg.context).length > 0) {
                    ...
                    if (msg.context.province)
                        telegramMessage += `- Tỉnh: ${msg.context.province.name}\n`;
                    if (msg.context.district)
                        telegramMessage += `- Huyện: ${msg.context.district.name}\n`;
                    if (msg.context.commune)
                        telegramMessage += `- Xã: ${msg.context.commune.name}\n`;
                }
                telegramMessage += `\n---\n\n`;
            });
            await sendTelegramAlert(telegramMessage);
            const ids = messages.map((m) => m.id);
            await queryWithCircuitBreaker(`UPDATE feedback SET is_sent_to_telegram = TRUE WHERE id = ANY($1::int[])`, [ids]);
